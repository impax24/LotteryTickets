This file contains instructions for how to build LotteryTickets for Ubuntu on Ubuntu Server 12.04 using
Oracle VirtualBox. When completed, you will be able to build the static loaded LotteryTicketsd using the
tested versions of libraries LotteryTickets needs to run properly. A fresh install of Ubuntu Server 12.04 is
...
build the GUI version of LotteryTickets. The windows version of the GUI gets much more attention from
...
Why would you want to build LotteryTickets this way?

      1 - It can be frustrating to build LotteryTickets for the first time. This will work every time.

      2 - Web servers usually have lots of added programs, libraries and configurations. Building
      LotteryTickets this way makes sure your LotteryTickets build is UN-tarnished.

      3 - LotteryTickets must be statically built with specific versions of libraries that are not usually
      included in Ubuntu.

      4 - Having your own Ubuntu build of LotteryTickets is a great way to enable web based LotteryTickets services
      that are built on Ubuntu servers to interact with LotteryTickets.

      5 - A stand alone VirtualBox running Ubuntu server 12.04 is a also a great way to run LotteryTickets
      in a secure environment, where it becomes much harder for anybody to steel your LotteryTickets. This
      might be especially important if you have a large stockpile of LotteryTickets that you like to get
      Proof of Stake (POS) rewards from.

      6 - Developing and debugging LotteryTickets may be easier for those who are more familiar with a Linux
...
Section A - Get Ubuntu Server 12.04.3 running inside VirtualBox with OpenSHH Server.
      ...
      3 - Create a virtual machine in VirtualBox. Click New. Give the VM a name, like "LotteryTickets
      ...
      we can SSH to it easily. Click on "LotteryTickets builder" and hit the "Settings" button. Click
      ...
      this server to store lots of LotteryTickets, you may choose to encrypt your home directory for extra
...
Section H - Get the latest production LotteryTickets source

      1 - Type " cd " to get to the home directory

      2 - Type " git clone https://github.com/LotteryTickets/LotteryTickets " to get the LotteryTickets source code.

Section I - Build the miniupnpc library

      1 - Type " cd " to get to the home directory

      2 - Type " cd LotteryTickets/src " miniupnpc goes here
...
Section J - Build LotteryTickets (LotteryTicketsd)

      1 - Type " cd " to get to the home directory

      2 - Type " cd LotteryTickets/src"

      3 - Type " ln -s makefile.ubuntu makefile " to make a symbolic link to the ubuntu makefile

      4 - Type " make "

Section K - Get the LotteryTicketsd executable file out of VirtualBox and running on a production
server.

      1 - Login to your server that you will use to run LotteryTicketsd.

      2 - Type " sudo apt-get install openssh-server " if your server does not already have OpenSSH
      server installed.

      3 - Login to the VirtualBox virtual server you used to build LotteryTicketsd.

      4 - Type " cd " to get to the home directory

      5 - Type " cd LotteryTickets/src "

      6 - Type " scp LotteryTicketsd yourusername@yourservername:. " to use SSH to copy the file onto your
      production server.

      7 - Go back to your production server.

      8 - Type " ./LotteryTicketsd -server -daemon " to attempt to start the daemon for the first time

      9 - Follow the instructions to create your rpcuser and rpcpassword in .LotteryTickets/LotteryTickets.conf

      10 - Type " ./LotteryTicketsd -server -daemon " to actually start the LotteryTicketsd daemon

      11 - Type " tail -f .LotteryTickets/debug.log " to watch the progress of LotteryTicketsd. Hit CTRL-C to stop.

      12 - Type " ./LotteryTicketsd getinfo " to see general info about the running daemon.

      13 - Type " ./LotteryTicketsd --help " to see a list of commands you can use while the daemon is
      running.

      14 - Type " ./LotteryTicketsd stop " when you need to stop the daemon.

Section L - Test LotteryTickets (LotteryTicketsd)
...
tests that don't compile for now.
      ...
      1 - Login to the VirtualBox virtual machine you used to build LotteryTicketsd.

      2 - Type " cd " to get to the home directory

      3 - Type " cd LotteryTickets/src"
      ...
      7 - Type " make test_LotteryTickets "

      8 - Type " ./test_LotteryTickets " to run the tests. Ouch! 110 failures detected! Now is your chance
          to contribute to LotteryTickets by fixing the broken tests.
